...
void MFRC522::PCD_Init() {
	...
	PCD_WriteRegister(RFCfgReg, (0x07<<4)); // Set Rx Gain to max
	    PCD_WriteRegister(GsNReg, 0xFF);        // Max conductance
    
    // Optimize antenna drivers for transmission
    PCD_WriteRegister(CWGsPReg, 0x3F);      // Max CW conductance
    PCD_WriteRegister(ModGsPReg, 0x3F); 
